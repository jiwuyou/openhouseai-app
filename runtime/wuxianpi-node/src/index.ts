...
if (process.argv.includes("--help") || process.argv.includes("-h")) {
  process.stdout.write("Usage: wuxianpi-node [--listen HOST:PORT] [--agent-dir PATH] [--idle-timeout-ms N]\n");
  process.exit(0);
}
...
const server = createRuntimeServer({ ...listen, idleTimeoutMs, agentDir });
await server.start();
