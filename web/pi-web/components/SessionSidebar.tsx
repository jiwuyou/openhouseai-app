...
import { runningPiSessionIds } from "@/lib/pi-websocket-transport";
...
export function SessionSidebar({ selectedSessionId, onSelectSession, onNewSession, initialSessionId, onInitialRestoreDone, refreshKey, onSessionDeleted, selectedCwd: selectedCwdProp, onCwdChange, onOpenFile, explorerRefreshKey, onAtMention }: Props) {
  ...
  useEffect(() => {
    const refresh = () => {
      ...
      setRunningSessionIds(new Set(runningPiSessionIds()));
    };
    refresh();
  ...
  }, [allSessions.length, handleDefaultCwd, initialSessionId, loading, selectedCwd]);
              ...
              {!customPathOpen ? (
                <button
                  onClick={(e) => {
                    ...
                    void fetchDefaultCwd().then((cwd) => setCustomPathValue((value) => value || cwd)).catch(() => {});
                    setCustomPathError(null);
