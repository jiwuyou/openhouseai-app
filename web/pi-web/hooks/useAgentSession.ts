...
import { bindAgentSession, connectAgentSession, sendAgentCommand } from "@/lib/agent-client";
import { getPiTransport, PiLeaseConflictError, type PiWebSocketTransport } from "@/lib/pi-websocket-transport";
...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  const eventSourceRef = useRef<PiWebSocketTransport | null>(null);
  ...
  const loadSession = useCallback(async (sid: string, showLoading = false, includeState = false) => {
    ...
    try {
      ...
      try {
        const transport = await connectAgentSession({ sessionId: sid, sessionPath: d.filePath });
        bindAgentSession(sid, transport);
        const liveState = await transport.send<AgentStateResponse>({ type: "get_state" });
        const agentState = { running: Boolean(liveState?.isStreaming || liveState?.isCompacting), state: liveState };
        if (sessionIdRef.current !== sid) return null;
  ...
  const ensureNewSession = useCallback(async () => {
    ...
    const promise = (async () => {
      ...
      const transport = await connectAgentSession({ cwd: newSessionCwd });
      const state = await transport.send<{ sessionId?: string }>({ type: "get_state" });
      const realId = state.sessionId;
      if (!realId) throw new Error("Pi runtime did not return a session id");
      bindAgentSession(realId, transport);
      eventSourceRef.current = transport;
      sessionIdRef.current = realId;
      if (selectedModel) {
        await transport.send({ type: "set_model", provider: selectedModel.provider, modelId: selectedModel.modelId });
      }
      if (thinkingLevel !== "auto") {
        await transport.send({ type: "set_thinking_level", level: thinkingLevel });
      }
      return realId;
    })();
  ...
  const connectEvents = useCallback(async (sid: string, sessionPath?: string) => {
    let transport: PiWebSocketTransport;
    try {
      transport = await connectAgentSession({ sessionId: sid, sessionPath });
    } catch (error) {
      const conflicted = getPiTransport(sid);
      if (!(error instanceof PiLeaseConflictError) || !conflicted
        || !window.confirm("This conversation is active in another client. Take over control?")) {
        throw error;
      }
      await conflicted.takeover();
      transport = conflicted;
    }
    bindAgentSession(sid, transport);
    eventSourceRef.current = transport;
    unsubscribeEventsRef.current?.();
    unsubscribeEventsRef.current = transport.subscribe((event) => handleAgentEventRef.current?.(event));
    return transport;
  }, []);

  const ensureEventsConnected = useCallback(async (sid: string) => {
    return connectEvents(sid, data?.filePath || session?.path);
  }, [connectEvents, data?.filePath, session?.path]);
  ...
  const reconcileAgentState = useCallback(async (sid: string) => {
    ...
    try {
      ...
      const busy = state && (state.isStreaming || state.isPromptRunning || state.isCompacting);
    ...
    switch (event.type) {
      ...
      case "agent_end":
        ...
        if (sessionIdRef.current) {
          ...
          sendAgentCommand<AgentStateResponse>(sessionIdRef.current, { type: "get_state" })
            .then((state) => {
              if (state.contextUsage !== undefined) setContextUsage(state.contextUsage ?? null);
              if (state.systemPrompt !== undefined) setSystemPrompt(state.systemPrompt ?? null);
              if (state.extensionStatuses !== undefined) setExtensionStatuses(state.extensionStatuses ?? []);
      ...
      case "extension_ui_request":
        ...
        break;
      case "runtime_resync": {
        const payload = event.messages as { messages?: AgentMessage[] } | AgentMessage[] | undefined;
        const recovered = Array.isArray(payload) ? payload : payload?.messages;
        if (recovered) setMessages(recovered.map(normalizeToolCalls));
        const state = event.state as AgentStateResponse | undefined;
        if (state) {
          setAgentRunning(Boolean(state.isStreaming || state.isPromptRunning));
          agentRunningRef.current = Boolean(state.isStreaming || state.isPromptRunning);
  ...
  }, [isNew, newSessionCwd, newSessionModel, newSessionDefaultModel, session, agentRunning, ensureNewSession, ensureEventsConnected, promoteNewSession, addNotice]);
  ...
  const handleFork = useCallback(async (entryId: string) => {
    ...
    try {
      ...
      if (!result?.cancelled) {
        const state = await sendAgentCommand<AgentStateResponse>(sid, { type: "get_state" });
        if (state.sessionId) {
          const transport = eventSourceRef.current;
          if (transport) bindAgentSession(state.sessionId, transport);
  ...
  const handleToolPresetChange = useCallback(async (preset: "none" | "default" | "full") => {
    // Tool availability comes from Pi's built-ins and loaded extensions. It
    // is intentionally not rewritten by the Web host.
    if (preset !== "full") {
      addNotice({ type: "info", message: "Tools are managed by the Pi runtime and its extensions." });
    }
    setToolPresetState("full");
      ...
      loadSession(session.id, true, true).then((agentState) => {
        if (agentState?.running) {
          ...
          if (agentState.state?.isStreaming || agentState.state?.isPromptRunning) {
            ...
            dispatch({ type: "start" });
            void connectEvents(session.id, session.path);
          }
        }
        if (agentState?.state) {
