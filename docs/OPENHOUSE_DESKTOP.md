...
- 桌面布局是稀疏槽位模型，保存每个 App 的 `pageIndex` 和 `slotIndex`，允许空位长期存在，不自动压紧。
- 桌面只保存入口元数据、槽位位置、显示名 override 和图标 override。
...
图标拖动后的页和槽位必须持久化。布局以稳定 App id 为 key 保存；某个 flavor 或版本中缺失的 App 要自动跳过但不破坏已保存布局，新增 App 要放入第一个可用空位；没有空位时追加新屏。

桌面不需要把图标紧凑排列。编辑模式下应显示可投放的空槽位：拖到空槽位时移动图标并留下原空位，拖到已占槽位时可以交换。图标可跨屏拖动；拖到最后一屏外侧时应创建新屏，并把图标放入新屏的目标槽位。
...
- 在桌面页，不显示顶部控制栏。
- 进入某个 App 后，显示顶部控制栏，结构为 `左侧栏 / 桌面 / 当前 App 名 / 刷新 / 收起 / 右侧控制栏`。
- `左侧栏` 打开全局 Drawer；`桌面` 返回原生桌面；`当前 App 名`展示当前页面或桌面 App 名；`刷新`只刷新当前 App；`右侧控制栏`打开当前 App 的状态、日志、服务控制和恢复动作。
- 顶部控制栏可以通过 `收起` 收起。收起后显示一个白色到黑色渐变的悬浮球。
- 悬浮球可以拖动，松手后吸附到左边或右边，并保存边和垂直位置；点击悬浮球展开控制栏。桌面页不显示悬浮球。
...
- 桌面 App 默认只显示图标和名称。
- 桌面支持横向分页、稀疏槽位、编辑模式、拖动排序、跨屏拖动、拖到末尾新建屏、改名和改图标。
- 桌面隐藏顶部控制栏；App 页显示 `左侧栏 / 桌面 / 当前 App 名 / 刷新 / 收起 / 右侧控制栏`。
- App 页控制栏可收起为白黑渐变悬浮球；悬浮球可拖动、吸附、持久化位置并点击展开。
- 打开失败自动弹状态面板。
