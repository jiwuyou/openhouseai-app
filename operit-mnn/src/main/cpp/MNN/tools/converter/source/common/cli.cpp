...
bool Cli::initializeMNNConvertArgs(modelConfig &modelPath, int argc, char **argv) {
    ...
    (
     ...
     "if 1, converter would align denormalized float(|x| < 1.18e-38) as zero, because of in linux/protobuf or android/flatbuf, system behaviors are different. default: 1, range: {0, 1}",
     cxxopts::value<int>()
     )
    (
     "detectSparseSpeedUp",
