...
for index in "${!targets[@]}"; do
  ...
  if grep -Eq 'service-manager 0\.(2\.1|3\.[0123])' "$target"; then
    fail "legacy service-manager gate remains in $target"
  fi

  actual_count="$(grep -Fc 'service-manager 0.3.4' "$target" || true)"
  if [ "$actual_count" -ne "$expected_count" ]; then
    fail "$target has $actual_count service-manager 0.3.4 gates; expected $expected_count"
  fi
done
...
(
  ...
  if service_manager_is_current "$work_dir/bin/service-manager"; then
    fail 'repair gate accepted previous 0.3.3'
  fi
  export FAKE_SERVICE_MANAGER_VERSION='service-manager 0.3.4'
  service_manager_is_current "$work_dir/bin/service-manager" \
    || fail 'repair gate rejected required 0.3.4'
)
...
(
  ...
  export FAKE_SERVICE_MANAGER_VERSION='service-manager 0.3.3'
  if find_termux_service_manager_binary >/dev/null; then
    fail 'start gate selected previous 0.3.3'
  fi
  export FAKE_SERVICE_MANAGER_VERSION='service-manager 0.3.4'
  [ "$(find_termux_service_manager_binary)" = "$work_dir/bin/service-manager" ] \
    || fail 'start gate did not select required 0.3.4'
)
