...
for index in "${!targets[@]}"; do
  ...
  if grep -Fq 'service-manager 0.2.0' "$target"; then
    fail "legacy 0.2.0 gate remains in $target"
  fi

  actual_count="$(grep -Fc 'service-manager 0.2.1' "$target" || true)"
  if [ "$actual_count" -ne "$expected_count" ]; then
    fail "$target has $actual_count service-manager 0.2.1 gates; expected $expected_count"
  fi
done
...
(
  ...
  if find_termux_service_manager_binary >/dev/null; then
    fail 'start gate selected legacy 0.2.0'
  fi
  export FAKE_SERVICE_MANAGER_VERSION='service-manager 0.2.1'
  [ "$(find_termux_service_manager_binary)" = "$work_dir/bin/service-manager" ] \
    || fail 'start gate did not select required 0.2.1'
)
