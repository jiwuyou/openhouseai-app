...
本文定义 OpenHouse 的“全部退出”语义。它不是关闭当前页面，也不是把 App 放到后台；它是用户主动要求停止 OpenHouse 相关前后台运行，让 CPU 和内存占用尽可能回到空闲。
...
如果某个服务是用户后来添加并注册为长期服务，也应被全部退出停止。
...
全部退出不是清理数据、不是重置系统、不是重装环境。

## 推荐执行顺序

1. UI 进入“正在全部退出”状态，暂停前台保活。
...
6. 只清理明确属于 OpenHouse 且无法正常停止的残留进程。
7. UI 显示“未运行”。
