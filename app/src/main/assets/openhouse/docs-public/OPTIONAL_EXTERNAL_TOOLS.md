...
默认核心能力是 service-manager、Codex CLI、Claude Code、CloudCLI、openhouse-connect 和 SmallPhone runtime。

OpenCode、Reasonix、Hermes 不作为 APK 内置脚本、payload、插件入口或默认服务打包。本文只作为产品手册，供 AI朋友 Help 或 AI 在用户明确要求时参考外部安装和配置；执行前应优先确认工具当前官方文档和版本要求。
...
不要在产品手册中保存真实 API key。AI朋友 Help 可以辅助生成本机配置，但必须让用户自行输入密钥，并避免写入日志。

AI朋友 Help 处理 Reasonix 请求时应先检查：
...
如果用户明确要求安装 Hermes，需要外部提供当前 Hermes 仓库、发布包或安装说明。AI朋友 Help 不应假设 APK 内有 Hermes 包。
...
- 默认核心问题：优先检查 service-manager、Codex CLI、Claude Code、CloudCLI、openhouse-connect、SmallPhone runtime。
