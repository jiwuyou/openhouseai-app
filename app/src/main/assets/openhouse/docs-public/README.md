...
- `OPENHOUSE_SYSTEM.md`：OpenHouse 系统 subject、索引渲染和系统检查命令。
- `PATHS_AND_PORTS.md`：正式路径和端口规范，说明稳定运行路径、端口分段、已用/保留端口和自定义 App 选端口规则。
- `MODEL_API_SETUP.md`：AionUI、Codex、Claude Code 和 CloudCLI 的模型/API 配置。
- `model-config-migration.md`：pi-web 模型配置迁移到 Claude Code、Codex、CloudCLI 的规则。
- `pi-agent-first-use.md`：`/openhouse-first-config` 标准模板的目标和验收；pi-agent 只是可选运行时。
...
- `OPENHOUSE_FIRST_CONFIGURATION.md`：固定安装后由当前 Agent 运行 `/openhouse-first-config` 完成首次 OpenHouse 配置的流程，并包含 AionUI 配置说明。
- `OPENHOUSE_HEALTH_SIGNOFF.md`：全面健康检查的一阶段/二阶段引导 AI 签名规则和二阶段复制提示词依据。
- `SECOND_AI_HANDOFF.md`：两个 Agent 应用或工作台如何生成交接文件、独立复核并继续任务；Agent identity 不是模型名称。

当前内测默认线是 pi-web 第一阶段检查系统，配置并真实测通 AionUI，再由 AionUI 第二阶段独立复核与签名。AionUI 的测通证据必须包括页面可打开、配置已保存、协议/Base URL/model id 可用，以及健康检查通过或真实对话收到有效响应。双签完成后再提供配置 Claude Code、配置 Codex、创建小型 Web App、跳过四个非阻断选项；通用任选 Agent 路径保留为高级或备用。

APK 还会把 `/openhouse-first-config`、`/openhouse-docs`、`/openhouse-second-ai-handoff` 三个标准模板安装到 `$PI_CODING_AGENT_DIR/prompts/`，实机默认是 Termux 的 `$HOME/.pi/prompts/`。模板内容、文档地址和交接要求由 App 自有 assets 在最终 payload 中维护；pi-web 的 OpenHouse 适配分支只提供产品入口并调用模板名，Android 不注入 prompt URL。
...
默认核心能力是 Termux、Termux 上的 Ubuntu、Node、OpenHouse 文档、openhouse-system、service-manager、OpenHouse Web、pi-agent、pi-web、openhouse-connect 和 SmallPhone 兼容服务。
