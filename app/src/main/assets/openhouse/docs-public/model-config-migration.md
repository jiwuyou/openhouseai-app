...
OpenHouseAI / AionUi 或 pi-web 已经测通的大模型配置可以作为后置工具配置的参考，但不能简单照抄。迁移时必须确认 provider、base URL、协议类型、model id、headers 和密钥来源。

迁移原则：

- 不在回复、日志、截图或文档中明文输出 API key、token、authorization header。
- 如果来源是 OpenHouseAI / AionUi，先读取并尊重 AionUi 自带协议检测、模型列表拉取、Key 测试和模型健康检查结果。
...
如果用户还没有在 OpenHouseAI / AionUi 中测通默认模型，应先阅读 `MODEL_API_SETUP.md`，让用户完成模型平台、模型供应商、base url、API Key、协议检测、模型列表拉取、模型 ID、New API 请求协议和模型健康检查，再继续迁移。
...
1. 从 AionUi 或 pi-web 已测通配置中提取 provider、base URL、model id 和协议信息。AionUi 配置字段重点是 `platform`、`name`、`base_url`、`api_key`、`models[]`、`model_protocols`、`model_health`。
