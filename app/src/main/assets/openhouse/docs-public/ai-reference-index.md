...
| SmallPhone Front Beta 部署 | `SMALLPHONE_FRONT_BETA_DEPLOY.md`, `SERVICE_MANAGER.md`, `PATHS_AND_PORTS.md` | 第二阶段 Agent（默认 AionUI）从市场 catalog 下载完整 `smallphone.tgz`，通过 Termux service-manager + Ubuntu/proot 部署、注册、启动，并按 endpoint 快照验收 `smallphone-core/api` 与 `smallphone-frontend-beta/web`；禁止五文件增量复制和固定端口回退。 |
