...
## 配置 Claude Code 时的可选用法

当用户要配置 CloudCLI 中的 Claude Code 时，pi-agent 可以把 cc-switch 作为可选配置执行器：

1. 先确认 `/root/.local/bin/claude`、CloudCLI 和 `cc/codex` 入口可用。
2. 执行 `/root/openhouse/scripts/install-cc-switch.sh` 安装或检查内置二进制。
3. 根据用户提供的 URL、key/token、model id 和协议类型创建或切换 Claude provider。
4. 必要时再按 `CLOUDCLI_CLAUDE_CODE.md` 修复 CloudCLI 使用的 `agent.js`、权限模式和服务配置。
5. 在 CloudCLI 页面中发起一次 Claude Code 任务，确认模型真实可用。

不要把 cc-switch 的成功退出码等同于 Claude Code 已经可用。最终验收仍以 CloudCLI 中的 Claude Code 能完成一次真实请求为准。
