...
桌面 App 图标默认不显示状态角标。点击是打开；长按空白处或图标进入桌面编辑模式，可在稀疏槽位中拖动、跨屏拖动、拖到末尾新建屏、改名、改图标、隐藏、重置、设为默认入口或进入状态/详情；空位不应自动压紧。打开失败或 WebView 主帧加载失败时，应自动进入状态面板。桌面页隐藏顶部控制栏；App WebView 页壳不重复显示 App 名，App 名只用于桌面、列表、服务、状态和无障碍。页壳保留侧栏、桌面、刷新、收起、控制和“用浏览器打开”，且可收起为可拖动、吸附并持久化位置的白黑渐变悬浮球。桌面 App 的入口类型可以是 `webview`、`native-page`、`terminal`、`service-control` 或 `android-activity`。桌面壳是原生 UI，不是全 WebView 多窗口系统，不应预创建多个 WebView；WebView 默认保留最近 2 个，可配置 0-5，按同 App/URL 复用和 LRU 清理，App 必须能从持久化状态恢复。
...
4. 先查 `/root/openhouse/docs`、`OPENHOUSE_SYSTEM.md`、`SERVICE_MANAGER.md`、`RECOVERY.md` 和 bootstrap 文档，确认是否有受支持的跨层命令。
...
9. 根据诊断结果选择 `OPENHOUSE_SYSTEM.md`、`SERVICE_MANAGER.md` 或 `RECOVERY.md` 中的最小修复步骤。
