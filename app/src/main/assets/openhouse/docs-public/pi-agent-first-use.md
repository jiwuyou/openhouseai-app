...
这是给 pi-web 首次配置提示词使用的稳定小写入口。`pi-agent` 可以作为候选运行时，但第一阶段执行者始终是当前实际会话中的 AI，不固定产品、模型或 agent identity。完整说明见同目录：
...
pi-web 首次使用的目标不是让用户再手工完成一堆配置，而是让当前会话中的 AI 先阅读 `$HOME/openhouse/docs`，理解 OpenHouse 的能力、架构、路径和排障入口，然后检查系统、准备用户选择的独立 AI，并生成接力任务。标准入口是 `/openhouse-first-config`；不要把第一阶段或第二阶段身份固定为某个产品或模型。

推荐流程：

1. 先确认 `service-manager`、`pi-web`、`pi-agent` 是否可用。
2. 阅读 `openhouse-overview.md`、`SERVICE_MANAGER.md`、`model-config-migration.md`、`CLOUDCLI_CLAUDE_CODE.md`、`TROUBLESHOOTING.md`。
...
2. 再阅读 `openhouse-overview.md`、`model-config-migration.md`、`CLOUDCLI_CLAUDE_CODE.md`、`codex-setup.md` 和 `SERVICE_MANAGER.md`。
