...
if is_current_ubuntu; then
  log "检测到当前已在 Ubuntu 内，直接安装或检查 Reasonix。"
  run_logged bash -s <<<"$(reasonix_install_program)"
elif command -v proot-distro >/dev/null 2>&1 && proot-distro login ubuntu -- true >/dev/null 2>&1; then
  log "正在 Ubuntu 内安装或检查 Reasonix。"
  run_logged proot-distro login ubuntu -- bash -s <<<"$(reasonix_install_program)"
else
