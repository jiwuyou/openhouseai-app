...
is_opencode_cwd_home() {
  run_ubuntu_logged bash -lc "set -euo pipefail; found=0; for pid in \$(pgrep -f 'opencode web.*--port $PORT' 2>/dev/null || true); do found=1; cwd=\$(readlink \"/proc/\$pid/cwd\" 2>/dev/null || true); if [ \"\$cwd\" = '/root' ]; then exit 0; fi; echo \"OpenCode 进程 \$pid 当前目录：\${cwd:-未知}\" >&2; done; [ \"\$found\" -eq 0 ] && echo '未找到 OpenCode 进程，准备重新拉起。' >&2; exit 1"
}
