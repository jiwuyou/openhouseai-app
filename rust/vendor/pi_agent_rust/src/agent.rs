...
impl AgentSession {
    ...
    /// Force-run compaction synchronously and stop promptly when aborted.
    pub async fn compact_now_with_abort(
        &mut self,
        abort: AbortSignal,
        on_event: impl Fn(AgentEvent) + Send + Sync + 'static,
    ) -> Result<()> {
        let on_event: AgentEventHandler = Arc::new(on_event);
        let compact_future = self.compact_synchronous(Arc::clone(&on_event));
        let abort_future = abort.wait();
        futures::pin_mut!(compact_future, abort_future);
        match futures::future::select(compact_future, abort_future).await {
            futures::future::Either::Left((result, _abort_future)) => result,
            futures::future::Either::Right(((), compact_future)) => {
                drop(compact_future);
