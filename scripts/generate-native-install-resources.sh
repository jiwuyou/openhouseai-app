...
cp "$payload_dir/pi-runtime.tar" "$stage/product-payloads/pi-runtime.tar"
...
runtime_sha="$(sha256sum "$stage/product-payloads/pi-runtime.tar" | awk '{print $1}')"
...
printf '%s  %s\n' \
  ...
  "$runtime_sha" product-payloads/pi-runtime.tar \
  "$bootstrap_sha" bootstrap/wuxianpi-setup \
  "$pre_tmux_sha" bootstrap/wuxianpi-pre-tmux.sh \
  > "$stage/SHA256SUMS"

printf '{"schema":1,"contents":["bootstrap","service-manager","wuxianpi-runtime"],"excluded":["aionui","pi-web","optional-products"]}\n' \
