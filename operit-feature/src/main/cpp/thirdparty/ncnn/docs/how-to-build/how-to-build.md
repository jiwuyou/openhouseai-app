...
On Debian, Linux, or Raspberry Pi OS, you can install all required dependencies using:
```shell
...
Alternatively install a cross-compiler provided by the distribution (i.e. on Debian / Linux, you can do `sudo apt install g++-arm-linux-gnueabi g++-arm-linux-gnueabihf g++-aarch64-linux-gnu`).
...
Install Termux on your phone and use the host-provided Android shell environment.
If you want to use ssh, install openssh in Termux.

Then make ncnn with the dependencies provided by that host environment.
...
- If you are using Linux, the official guidelines from DevkitPro might not work for you. Try using the lines below to install
```shell
sudo apt-get update
