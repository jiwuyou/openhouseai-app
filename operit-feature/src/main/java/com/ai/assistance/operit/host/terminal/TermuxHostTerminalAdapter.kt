...
class TermuxHostTerminalAdapter {
    ...
    suspend fun initialize(): Boolean {
        if (OperitHostProvider.currentOrNull() == null) {
            return false
        }
        refreshState()
        return true
    ...
    fun isConnected(): Boolean = OperitHostProvider.currentOrNull() != null
        ...
        val execution =
            withContext(Dispatchers.IO) {
                runCatching {
                    ...
                    executeWithTargetFallback(host, session.target, commandToRun, effectiveTimeoutMs)
                }
                    .getOrElse { throwable ->
                        HostExecution(
                            result =
    ...
    private suspend fun executeWithTargetFallback(
        host: com.ai.assistance.operit.host.OperitHostContract,
        requestedTarget: HostTerminalTarget,
        command: String,
        timeoutMs: Long
    ): HostExecution {
        return when (requestedTarget) {
            HostTerminalTarget.AUTO -> executeAutoTarget(host, command, timeoutMs)
            HostTerminalTarget.UBUNTU ->
                HostExecution(host.executeUbuntuCommand(command, timeoutMs), HostTerminalTarget.UBUNTU)
            HostTerminalTarget.TERMUX ->
                HostExecution(host.executeTermuxCommand(command, timeoutMs), HostTerminalTarget.TERMUX)
        }
    }

    private suspend fun executeAutoTarget(
        host: com.ai.assistance.operit.host.OperitHostContract,
        command: String,
        timeoutMs: Long
    ): HostExecution {
        val ubuntuProbe =
            runCatching {
                    host.executeUbuntuCommand(UBUNTU_READY_PROBE_COMMAND, UBUNTU_PROBE_TIMEOUT_MS)
                }
                .getOrElse { throwable ->
                    OperitHostCommandResult(
                        command = UBUNTU_READY_PROBE_COMMAND,
                        exitCode = -1,
                        stdout = "",
                        stderr = "",
        ...
        if (ubuntuProbe.isSuccess && ubuntuProbe.stdout.contains(UBUNTU_READY_MARKER)) {
            return HostExecution(host.executeUbuntuCommand(command, timeoutMs), HostTerminalTarget.UBUNTU)
        }

        val fallbackNotice = buildAutoFallbackNotice(ubuntuProbe)
        val termuxResult = host.executeTermuxCommand(command, timeoutMs)
        return HostExecution(withFallbackNotice(termuxResult, fallbackNotice), HostTerminalTarget.TERMUX)
    }

    private fun buildAutoFallbackNotice(ubuntuProbe: OperitHostCommandResult): String {
        val reason =
            listOf(ubuntuProbe.error, ubuntuProbe.stderr, ubuntuProbe.stdout)
    ...
    private fun withFallbackNotice(
        result: OperitHostCommandResult,
        notice: String
    ): OperitHostCommandResult {
        return if (result.isSuccess) {
            result.copy(stdout = prependNotice(result.stdout, notice))
        } else {
            result.copy(stderr = prependNotice(result.stderr, notice))
        }
    }

    private fun prependNotice(text: String, notice: String): String {
    ...
    companion object {
        ...
        private const val UBUNTU_PROBE_TIMEOUT_MS = 5_000L
        private const val UBUNTU_READY_MARKER = "__OPERIT_UBUNTU_READY__"
        private const val UBUNTU_READY_PROBE_COMMAND = "printf __OPERIT_UBUNTU_READY__"
    }
}
