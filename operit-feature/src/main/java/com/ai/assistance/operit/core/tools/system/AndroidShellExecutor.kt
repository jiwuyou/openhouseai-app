...
class AndroidShellExecutor {
    companion object {
        ...
        fun clearPreferredPermissionLevelCache() {
            // Hosted mode ignores legacy shell permission preference and always uses Termux host.
        }
        ...
        suspend fun executeShellCommand(command: String, identityOverride: ShellIdentity?): CommandResult {
            ...
            val standardExecutor = ShellExecutorFactory.getExecutor(ctx, AndroidPermissionLevel.STANDARD)
            val permStatus = standardExecutor.hasPermission()
            val executorAvailable = standardExecutor.isAvailable()

            if (executorAvailable && permStatus.granted) {
                val result = standardExecutor.executeCommand(command, ShellIdentity.DEFAULT)
                return CommandResult(result.success, result.stdout, result.stderr, result.exitCode)
            }

            val reason =
                    buildStrictUnavailableReason(AndroidPermissionLevel.STANDARD, executorAvailable, permStatus)

            AppLogger.d(TAG, "Hosted terminal mode enabled. $reason")
        ...
        suspend fun startShellProcess(command: String): ShellProcess {
            ...
            if (executorAvailable && permStatus.granted) {
                return standardExecutor.startProcess(command)
            }
