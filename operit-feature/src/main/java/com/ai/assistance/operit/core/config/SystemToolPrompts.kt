...
object SystemToolPrompts {
    ...
    private val hostTerminalToolOrder = hostTerminalToolNames
        .withIndex()
        .associate { (index, name) -> name to index }

    private fun buildHostTerminalToolCategory(useEnglish: Boolean): SystemToolPromptCategory {
        val categories = if (useEnglish) {
            internalToolCategoriesEn
        } else {
            internalToolCategoriesCn
        }
        val tools = categories
            .asSequence()
            .flatMap { it.tools.asSequence() }
            .filter { it.name in hostTerminalToolNames }
            .distinctBy { it.name }
            .sortedBy { hostTerminalToolOrder[it.name] ?: Int.MAX_VALUE }
            .toList()

        return SystemToolPromptCategory(
            categoryName = if (useEnglish) "Termux Terminal Tools" else "Termux 终端工具",
            tools = tools
        )
    }
...
""".trimEnd()
    ...
    fun getHostTerminalToolCategoryEn(): SystemToolPromptCategory = buildHostTerminalToolCategory(useEnglish = true)

    fun getHostTerminalToolCategoryCn(): SystemToolPromptCategory = buildHostTerminalToolCategory(useEnglish = false)
    ...
    fun getManageableToolPrompts(useEnglish: Boolean): List<ManageableToolPrompt> {
        val baseCategories = if (useEnglish) {
            listOf(basicTools, fileSystemTools, httpTools, memoryTools, getHostTerminalToolCategoryEn())
        } else {
            listOf(basicToolsCn, fileSystemToolsCn, httpToolsCn, memoryToolsCn, getHostTerminalToolCategoryCn())
        }
