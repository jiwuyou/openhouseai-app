        ...
        return try {
            ...
            when (element) {
                ...
                is JsonObject -> parseObjectFormat(element)
                    ...
                    OperitRuntimeContext.getStringOrDefault(
                        R.string.generic_json_unsupported_format,
                        "不支持的 JSON 格式"
                    )
                )
            }
            ...
            throw ConversionException(
                OperitRuntimeContext.getStringOrDefault(
                    R.string.generic_json_parse_failed,
                    "解析通用 JSON 格式失败: %s",
                    e.message ?: ""
                ),
                e
            ...
            if (isConversationObject(firstElement)) {
                ...
                if (messages.isNotEmpty()) {
                    listOf(ChatHistory(
                        ...
                        group = OperitRuntimeContext.getStringOrDefault(
                            R.string.generic_json_import_from,
                            "从通用格式导入"
                        )
                    ))
                } else {
