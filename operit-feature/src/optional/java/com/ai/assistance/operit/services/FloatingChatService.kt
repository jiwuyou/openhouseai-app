...
class FloatingChatService : Service(), FloatingWindowCallback {
    ...
    private fun handleServiceCrash(thread: Thread, throwable: Throwable) {
        try {
            ...
            saveState()
            AppLogger.w(TAG, "Floating service crash captured; restart is delegated to host lifecycle policy")
        } catch (e: Exception) {
    ...
    override fun onCreate() {
        super.onCreate()
        initializeHostApplicationIfAvailable()
    ...
    private fun initializeHostApplicationIfAvailable() {
        runCatching {
            application.javaClass.methods
                .firstOrNull { method ->
                    method.name == "initializeMainApplication" && method.parameterTypes.isEmpty()
                }
                ?.invoke(application)
        }.onFailure { error ->
            AppLogger.w(TAG, "Host application initialization hook unavailable", error)
        }
    }
    ...
    override fun onTaskRemoved(rootIntent: Intent) {
        ...
        AppLogger.i(TAG, "Task removed; host lifecycle policy controls whether the floating service restarts")
    }
